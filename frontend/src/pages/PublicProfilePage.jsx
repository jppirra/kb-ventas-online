...
function ProductCard({ product, catalogName, vendorWhatsapp, primaryColor, onWhatsappClick }) {
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <button
          ...
          className="mt-3 w-full py-2 rounded-xl text-white text-xs font-semibold flex items-center justify-center gap-1.5 transition-opacity hover:opacity-90"
...
function CatalogSection({ catalog, vendorWhatsapp, primaryColor, onWhatsappClick, onView }) {
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1">
          ...
          <button onClick={() => setViewMode('CAROUSEL')}
            className={`p-1.5 rounded-lg transition-colors ${viewMode === 'CAROUSEL' ? 'bg-gray-200 dark:bg-slate-600' : 'hover:bg-gray-100 dark:hover:bg-slate-700'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-600 dark:text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {catalog.products.map(p => (
            <ProductCard key={p.id} product={p} catalogName={catalog.name}
              vendorWhatsapp={vendorWhatsapp} primaryColor={primaryColor} onWhatsappClick={onWhatsappClick} />
          ))}
        </div>
      ) : (
...
export default function PublicProfilePage() {
  ...
  const primary = profile.brandColorPrimary || '#2563eb'
  const secondary = profile.brandColorSecondary || '#7c3aed'
